...
> 登录采用统一认证方式，用户只需按页面提示完成登录与授权，无需配置技术细节。
...
## 登录与安全说明
- 采用统一认证登录，浏览器自动跳转到授权页并回到系统，无需额外配置。
- 登录状态自动保持，关闭浏览器后建议重新登录以确保安全。
- 建议绑定手机号与邮箱，便于找回密码和接收安全通知。

---

## 常见问题
- **登录后空白或无权限提示**：请确认使用的是有效项目与组织，并联系管理员确认账号授权。
- **应用不显示**：请确认该项目已添加对应应用，如仍不可见请联系管理员为账号开通权限。
